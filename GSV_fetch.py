...
# Example: C:\Project\data\outputs  C:\Project\data\Tree_locations.shp AIzaSyDNnEDpuoe1sWO7w6UB4Td4rEWdZRD4fXA
...
def fetchStreetViews(treeLocations, outWorkspace, API_key):
    ...
    if not os.path.exists(treeLocations_proj):
    ...
    # Creating the Image tagged Shapefile
    ...
    heading = 0
    GSV_Filename_List = []
    downloadedGSVLocations = []
    # for point in range(len(longitude) - 1, 0, -1):
    for point in range(10762, 0, -1):  # Either len(longitude) or len(latitude) can be used here
        print point
        photo_details = accessMetadata(point, API_key, workSpace, latitude,
                                       longitude)  # Acessing metadata to check photo details
        time.sleep(0.100)
        if photo_details is not None:  # This is to ignore corrupted or haphazard metadata file
            date_photo = photo_details[0]
            latitude_photo = photo_details[1]
            longitude_photo = photo_details[2]
            month_photo = photo_details[3]

            if (month_photo in ['01', '02', '03', '11', '12'] and ((latitude_photo,
                                                                          longitude_photo) not in downloadedGSVLocations)):  # Checking if photos belong to winter (Nov 1st-Apr 30th)
                downloadedGSVLocations.append((latitude_photo, longitude_photo))
                for pitch in range(0, 91, 45):
                    for heading in range(0, 360, 60):
                        url = "https://maps.googleapis.com/maps/api/streetview?location={0},{1}&size=640x640&heading={2}&pitch={3}&fov=120&key={4}".format \
                            (latitude[point], longitude[point], heading, pitch, API_key)

                        GSV_Filename = "GSV_{0}_{1}_{2}_{3}.jpg".format(latitude[point], longitude[point], heading,
                                                                        pitch)
                        ...
                        time.sleep(
                            0.501)  # GSV API will blacklist the user if the frequency of requests is very high. This code pauses the execution for 501 milliseconds

                printArc("Street view images from a point location successfully downloaded to local disk")
                writeMetadata(point, API_key, workSpace, latitude, longitude)
    ...
    # Reprojecting the outputshapefile back to the original projection system of the tre locations input file
...
def accessMetadata(point, API_key, workSpace, latitude, longitude):
    ''''''
    metadata_url = "https://maps.googleapis.com/maps/api/streetview/metadata?location={0},{1}&key={2}".format(
        latitude[point], longitude[point], API_key)
    response = urllib.urlopen(metadata_url)
    data = json.loads(response.read())
    if data['status'] != 'NOT_FOUND' or data['status'] != 'ZERO_RESULTS':
        try:
            longitude_photo = (data['location']['lng'])
            latitude_photo = (data['location']['lat'])
            date_photo = data['date']
            month_photo = date_photo[-2:]
            return date_photo, latitude_photo, longitude_photo, month_photo
        except:
            return


def writeMetadata(point, API_key, workSpace, latitude, longitude):
    ...
    if data['status'] != 'NOT_FOUND':
        ...
        GSV_metadata_Filename = "GSVmetadata/GSV_{0}_{1}.json".format(latitude[point], longitude[point])
...
def insertGSVintoShapefile(date_photo, latitude_photo, longitude_photo, GSV_Filename_List, outShapefile):
    ...
    try:
        ...
        ic = arcpy.da.InsertCursor(outShapefile, ['Date', 'Latitude', 'Longitude', \
                                                  ...
                                                  'p90h0', 'p90h60', 'p90h120', 'p90h180', 'p90h240', 'p90h300',
                                                  'SHAPE@XY'])
